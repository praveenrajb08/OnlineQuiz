...
try
{
	...
	{
		String dbemail,dbphone;
		
		String email,phonenumber,new_password;
		
		email=request.getParameter("txt_email"); //txt_email
		phonenumber=request.getParameter("phonenumber"); //phonenumber
		new_password=request.getParameter("new_password");//new_password
		PreparedStatement pstmt=null; //create statement
		
		pstmt=con.prepareStatement("select * from login where email=? AND phonenumber=?"); //sql select query 
		...
		if(rs.next())
		{
			...
			dbphone=rs.getString("phonenumber");
			
			if(email.equals(dbemail) && phonenumber.equals(dbphone))
			{
				//session.setAttribute("login",dbemail); //session name is login and store fetchable database email address
				Statement st1=con.createStatement();
				int i=st1.executeUpdate("update login set password='"+new_password+"' where email='"+email+"'");
		...
		{
			request.setAttribute("errorMsg","invalid email or phonenumber"); //invalid error message for email or password wrong
		}
...
<body>
    ...
    <div class="main-content">
		...
		<a href="index.jsp"><button name="btn_login">back</button></a>
